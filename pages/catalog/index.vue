...
const minPrice = ref(Number(readQuery('min')) || 0)
const maxPrice = ref(Number(readQuery('max')) || 0)
...
const productQuery = computed(() => ({
  ...
  subcategoryId: subcategory.value || undefined,
  collectionId: collection.value || undefined,
  availability: availability.value || undefined,
  colorName: color.value || undefined,
  minPrice: minPrice.value || undefined,
  maxPrice: maxPrice.value || undefined,
  sort: sort.value === 'default' ? undefined : sort.value,
  limit: page.value * perPage,
}))
const { data: productData, status: productStatus, refresh: refreshProducts } = await useAsyncData<ProductListResponse>(
  `catalog-products-${route.fullPath}`,
  () => requestFetch<ProductListResponse>('/api/catalog/product-list', { query: productQuery.value }),
)
const priceFloor = computed(() => productData.value?.facets.priceFloor ?? 0)
const priceCeil = computed(() => productData.value?.facets.priceCeil ?? 0)
if (!minPrice.value) minPrice.value = priceFloor.value
if (!maxPrice.value) maxPrice.value = priceCeil.value
const allColors = computed(() => productData.value?.facets.colors ?? [])
const visibleProducts = computed(() => productData.value?.items ?? [])
const totalProducts = computed(() => productData.value?.total ?? 0)
const activeCategory = computed(() => categories.value.find(item => item.id === category.value))
const visibleSubcategories = computed(() => activeCategory.value?.subcategories ?? categories.value.flatMap(item => item.subcategories))
const visibleCollections = computed(() => category.value ? collections.value.filter(item => item.categoryId === category.value) : collections.value)
const activeFilters = computed(() => [category.value, subcategory.value, collection.value, availability.value, color.value].filter(Boolean).length + Number(minPrice.value !== priceFloor.value || maxPrice.value !== priceCeil.value))

const syncQuery = () => {
  const query: Record<string, string> = {}
  if (minPrice.value !== priceFloor.value) query.min = String(minPrice.value)
  if (maxPrice.value !== priceCeil.value) query.max = String(maxPrice.value)
  ...
  <div class="container-page py-10 md:py-14">
    ...
    <section aria-labelledby="products-heading">
      <div class="mb-6 flex flex-wrap items-center justify-between gap-4 border-y border-border py-4">
        <div><h2 id="products-heading" class="text-2xl font-light">Товары и модули</h2><p class="text-xs text-secondary">Найдено: {{ totalProducts }}</p></div>
      ...
      <div class="grid gap-8 lg:grid-cols-[260px_1fr]">
        ...
        <div>
          ...
          <div v-if="visibleProducts.length < totalProducts" class="mt-12 text-center"><button type="button" class="btn-secondary" :disabled="productStatus === 'pending'" @click="page += 1">{{ productStatus === 'pending' ? 'Загрузка…' : 'Показать ещё' }}</button></div>
        </div>
      </div>
    </section>
  </div>
